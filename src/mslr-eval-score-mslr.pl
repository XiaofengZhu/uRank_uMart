...
# output the real query id
        ...
        if ($flag == 1)
        {
            ...
            {   if ($iPos == 0 or $iPos == 2 or $iPos == 4 or $iPos == 9)
                {
                    print FOUT sprintf("%.3f\t", $ndcg_q[$iPos]);
                }
            }
            # print FOUT sprintf("%.4f\n", $meanNdcg_q);
        }
    ...
    for(my $iPos = 0; $iPos < $iMaxPosition; $iPos ++)
    {
        ...
        {
            print FOUT sprintf(" ndcg_%d: %.3f ;", $iPos+1, $ndcg[$iPos]);
        }
        if ($iPos == 9)
        {
            print FOUT sprintf(" ndcg_%d: %.3f", $iPos+1, $ndcg[$iPos]);
    ...
    for(my $iPos = 0; $iPos < $iMaxPosition; $iPos ++)
    {
        ...
        if ($iPos == 0 or $iPos == 2 or $iPos == 4 or $iPos == 9)
        {
            print FOUT sprintf(" ; err_%d: %.3f", $iPos+1, $err[$iPos]);
        }        
    }    
    # $meanNdcg /= ($#qids + 1);
    # print FOUT sprintf("%.4f\n\n", $meanNdcg);
...
# modified by Jun Xu, calculate all possible positions' NDCG for MeanNDCG
        ...
        my @Err = ERR($iMaxPosition, @rates);# modified
        my $meanErr = 0;
        for(my $iPos = 0; $iPos < $#Err + 1; $iPos ++)
        {
            $meanErr += $Err[$iPos];
        }
        $meanErr /= ($#Err + 1);
        ...
        @{$hsResults{$qid}{"ERR"}} = @Err;
        $hsResults{$qid}{"MeanERR"} = $meanErr;
    }
    return %hsResults;
}
...
sub ERR
{
    ...
    my @err;
    my $max = max @rates;
    my $maxG = $hsErrRelScore{$max};
    my $prob_step_down = 1.0;
    my $utility = $hsNdcgRelScore{$rates[0]} / $maxG;
    $err[0] = $utility;
    $prob_step_down = $prob_step_down * (1 - $utility);
    for(my $iPos = 1; $iPos < $topN; $iPos ++)
    {
        my $r;
        if ($iPos < $#rates + 1)
        {
            $r = $rates[$iPos];
            $utility = $hsNdcgRelScore{$r} / $maxG;
            $err[$iPos] = $err[$iPos - 1] + $prob_step_down * $utility / ($iPos + 1);
            $prob_step_down = $prob_step_down * (1 - $utility);            
        }
        else
        {
